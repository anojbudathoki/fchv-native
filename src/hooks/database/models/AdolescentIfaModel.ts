...
import { CreateAdolescentIfaPayload, AdolescentIfaStoreType } from '../types/adolescentIfaModal';

const WEEK_COLS_PHASE1 = Array.from({ length: 13 }, (_, i) => `phase1_week_${i + 1}`);
const WEEK_COLS_PHASE2 = Array.from({ length: 13 }, (_, i) => `phase2_week_${i + 1}`);
const ALL_WEEK_COLS = [...WEEK_COLS_PHASE1, 'phase1_completed', ...WEEK_COLS_PHASE2, 'phase2_completed'];
...
export async function createAdolescentIfa(
  ...
  const cols = ['id', 'name', 'age_group', ...ALL_WEEK_COLS, 'remarks', 'is_synced', 'is_deleted', 'created_at', 'updated_at'];
  const placeholders = cols.map(() => '?').join(', ');

  const updateSet = cols
    .filter(c => c !== 'id' && c !== 'created_at' && c !== 'is_synced' && c !== 'is_deleted')
    .map(c => `${c} = excluded.${c}`)
    .join(', ');
...
export async function getAdolescentIfaById(id: string): Promise<AdolescentIfaStoreType | null> {
  const db = await getDb();
  return await db.getFirstAsync<AdolescentIfaStoreType>(
    `SELECT * FROM adolescent_ifa WHERE id = ? AND is_deleted = 0`, [id]
  );
}
