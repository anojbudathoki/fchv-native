...
export async function getCounselingByMother(
  ...
  let query = `SELECT * FROM counseling WHERE mother_id = ? AND reg_year = ? AND reg_month = ? AND is_deleted = 0`;
  ...
  const result = await db.getFirstAsync<CounselingStoreType>(query, params);
  return result || null;
}
...
export async function saveCounseling(payload: {
  ...
  try {
    const existing = await getCounselingByMother(payload.mother_id, payload.pregnancy_id, regYear, regMonth);
    ...
    if (existing) {
      ...
      await db.runAsync(
        `INSERT OR REPLACE INTO counseling (id, mother_id, pregnancy_id, is_counseled, counseled_topics, reg_year, reg_month, created_at, updated_at, is_synced, is_deleted) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)`,
        [id, payload.mother_id, payload.pregnancy_id ?? null, payload.is_counseled, payload.counseled_topics ?? null, regYear, regMonth, now, now],
      );
      return {
        id,
    ...
    console.error("saveCounseling failed:", error);
    throw error;
  }
}
