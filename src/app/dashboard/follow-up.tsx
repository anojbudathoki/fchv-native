...
export default function FollowUpForm() {
  const { addVisit, isLoading } = useVisit();
  ...
  const [errors, setErrors] = useState<{ name?: string; date?: string; visitType?: string }>({});
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <NavigationLayout title="Follow-up Visit" />
                ...
                {VISIT_TYPES.map((type, index) => (
                  <TouchableOpacity
                    ...
                    className={`px-4 py-3.5 flex-row justify-between items-center ${
                      index < VISIT_TYPES.length - 1 ? 'border-b border-gray-100' : ''
