...
export default function ChildMonitoringDetailsScreen() {
  ...
  const getBirthPlaceDisplay = () => {
    switch (data.birth_place) {
      case "home":
        return { label: t("reports.child_monitoring_report.home"), img: house };
      case "institution":
        return {
          label: t("reports.child_monitoring_report.facility"),
          img: hospital,
        };
      case "trained_worker":
        return {
          label: t("reports.child_monitoring_report.health_worker"),
          img: fchv,
        };
      default:
  ...
  const CategoryItem = ({
    ...
    <View className="flex-row items-center justify-between p-4 bg-white border-b border-slate-100">
      <View className="flex-row items-center flex-1 pr-4">
        <Image
          source={icon}
          className="w-12 h-12 rounded-lg border border-slate-200 mr-4"
          resizeMode="cover"
        />
        <Text className="text-slate-800 font-medium flex-1 leading-snug">
          {title}
        </Text>
      </View>
    ...
    <SafeAreaView className="flex-1 bg-[#F8FAFC]">
      ...
      >
        ...
        <View className="mx-4 bg-white p-5 rounded-2xl shadow-sm shadow-slate-200/50 border border-slate-100 mb-6">
          ...
          <View className="flex-row gap-4">
            <View className="flex-1 bg-slate-50 p-3 rounded-xl flex-row gap-1 items-center">
              ...
              <View>
                <Text className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-0.5">
                  DOB (BS)
                </Text>
                <Text className="text-sm font-semibold text-slate-800">
                  {formatBsDate(data.date_of_birth)}
                </Text>
              </View>
            </View>
        ...
        <View className="mx-4 bg-white rounded-2xl shadow-sm shadow-slate-200/50 border border-slate-100 overflow-hidden mb-6">
          ...
          <View>
            <CategoryItem
              title={t("reports.child_monitoring_report.umbilical_ointment")}
              icon={Umbilical}
              value={!!data.umbilical_ointment}
            />
            <CategoryItem
              title={t("reports.child_monitoring_report.skin_to_skin")}
              icon={NewBornBabyTouchWithChest}
              value={!!data.skin_to_skin}
            />
            <CategoryItem
              title={t("reports.child_monitoring_report.early_breastfeeding")}
              icon={breastfeeding}
              value={!!data.early_breastfeeding}
            />

            <CategoryItem
              title={t("reports.child_monitoring_report.normal_weight")}
              icon={babyWeighting}
              value={data.baby_weight === "normal"}
            />
            <CategoryItem
              title={t("reports.child_monitoring_report.low_weight")}
              icon={babyWeighting}
              value={data.baby_weight === "low"}
              isNegative
            />
            <CategoryItem
              title={t("reports.child_monitoring_report.very_low_weight")}
              icon={babyWeighting}
              value={data.baby_weight === "very_low"}
              isNegative
            />

            <CategoryItem
              title={t("reports.child_monitoring_report.asphyxiated")}
              icon={breathing}
              value={!!data.asphyxiated_newborn}
              isNegative
            />
          </View>
        </View>
        ...
        <View className="mx-4 bg-white rounded-2xl shadow-sm shadow-slate-200/50 border border-slate-100 overflow-hidden">
          ...
          <View className="p-4 flex-row gap-4">
            <View
              className={`flex-1 p-4 rounded-xl border ${data.skilled_birth_attended ? "bg-emerald-50 border-emerald-100" : "bg-slate-50 border-slate-100"} items-center`}
            >
              <Text
                className={`font-bold mb-2 text-center ${data.skilled_birth_attended ? "text-emerald-700" : "text-slate-500"}`}
              >
                {t("reports.child_monitoring_report.trained_worker")}
              </Text>
              {data.skilled_birth_attended ? (
            ...
            <View
              className={`flex-1 p-4 rounded-xl border ${data.fchv_present ? "bg-emerald-50 border-emerald-100" : "bg-slate-50 border-slate-100"} items-center`}
            >
              <Text
                className={`font-bold mb-2 text-center ${data.fchv_present ? "text-emerald-700" : "text-slate-500"}`}
              >
