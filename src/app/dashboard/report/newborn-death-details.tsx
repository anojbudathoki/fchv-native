...
export default function NewbornDeathDetailsScreen() {
  ...
  const getBirthConditionLabel = () => {
    switch (data.birth_condition) {
      case "Preterm":
        return t("newborn_death_modal.preterm");
      case "LowWeight":
        return t("newborn_death_modal.low_weight");
      case "Normal":
        return t("newborn_death_modal.normal");
      case "Other":
        return data.birth_condition_other || t("newborn_death_modal.other");
      default:
        return "-";
    }
  };
  ...
  const getCauseLabel = () => {
    switch (data.cause_of_death) {
      case "Asphyxia":
        return t("reports.newborn_death_report.asphyxia");
      case "Hypothermia":
        return t("reports.newborn_death_report.hypothermia");
      case "Infection":
        return t("reports.newborn_death_report.infection");
      case "Pneumonia":
        return "Pneumonia";
      case "Diarrhea":
        return "Diarrhea";
      case "Malnutrition":
        return "Malnutrition";
      case "Other":
        return data.cause_of_death_other || t("newborn_death_modal.other");
      default:
        return "-";
    }
  };
  ...
  const getDeathPlaceLabel = () => {
    switch (data.death_place) {
      case "Home":
        return t("newborn_death_modal.home");
      case "Institution":
        return t("newborn_death_modal.institution");
      case "Other":
        return data.death_place_other || t("newborn_death_modal.other");
      default:
        return "-";
    }
  };
  ...
  const getGenderLabel = () => {
    ...
    if (data.gender === "Female")
      return t("reports.newborn_death_report.female");
    return "-";
  };
  ...
  const InfoRow = ({ label, value, icon: Icon, color = "#64748B" }: any) => (
    <View className="flex-row items-start mb-4">
      ...
      <View className="flex-1 ml-2">
        <Text className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">
          {label}
        </Text>
          ...
          <View className="flex-1">
            ...
            <View className="flex-row items-center mt-1 gap-2">
              <View
                ...
                style={{
                  backgroundColor:
                    data.gender === "Male" ? "#EFF6FF" : "#FDF2F8",
                }}
              >
                <Text
            ...
            {motherData && (
              <>
                <InfoRow
                  label="Phone Number"
                  value={
                    motherData.phone_number || motherData.partnerMobile || "-"
                  }
                  icon={Info}
                  color="#10B981"
                />
        ...
        {!!data.remarks && (
          <View className="mx-4 bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <View className="bg-slate-50 px-5 py-3 border-b border-slate-100 flex-row items-center">
              ...
              <Text className="text-slate-700 font-bold text-sm uppercase tracking-wider">
                {t("newborn_death_modal.remarks")}
              </Text>
            </View>
            <View className="p-5">
              <Text className="text-slate-700 text-[15px] leading-relaxed font-medium">
                {data.remarks}
              </Text>
            </View>
          </View>
